...
void multiboot_init(uint32_t magic, uint32_t addr)
{
    ...
    boot_info.root_dev = 0x301;

    if (mbi->flags & MB_FLAG_CMDLINE) {

        uint16_t cmd_root =
            parse_root_from_cmdline((char*)mbi->cmdline);
    ...
    if (mbi->flags & MB_FLAG_MODS) {
        ...
        multiboot_module_t *found = NULL;

        for (uint32_t i = 0; i < mbi->mods_count; i++) {

            char *name = (char*)mods[i].string;
            uint32_t size = mods[i].mod_end - mods[i].mod_start;
            uint32_t size_kb = size / 1024;

            // Новый формат вывода
            printk("module %d: %s 0x%x, %d KB\n",
                   i,
                   name ? name : "<none>",
                   mods[i].mod_start,
                   size_kb);
            ...
            if (!found && name) {
                ...
                if (strcmp(name, "initrd") ||
                    strcmp(name, "initramfs") ||
                    strcmp(name, "ramdisk"))
                    found = &mods[i];
            }
        }

        if (!found && mbi->mods_count == 1)
            found = &mods[0];

        if (found) {

            initrd_start = found->mod_start;
            initrd_end   = found->mod_end;
            printk("Initrd selected, ");
