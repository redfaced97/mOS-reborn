...
static void scroll() {
    ...
    for (int x = 0; x < VGA_WIDTH; x++)
        video[(VGA_HEIGHT - 1) * VGA_WIDTH + x] = (0x07 << 8) | ' ';
}
...
static char *number(char *str, unsigned long num, int base, int width, int precision, int flags) {
    char tmp[32];
...
int vsprintf(char *buf, const char *fmt, va_list args) {
    ...
    while (*fmt) {
        ...
        int flags = 0, width = -1, precision = -1;
        fmt++;
        ...
        case 'u':
            ...
            break;
        case 'x': flags |= SMALL; /* fallthrough */
        case 'X':
            str = number(str, va_arg(args, long), 16, width, precision, flags);
            break;
        case '%': *str++ = '%'; break;
        default: *str++ = '%'; *str++ = *fmt; break;
        }
        fmt++;
    }
    *str = '\0';
